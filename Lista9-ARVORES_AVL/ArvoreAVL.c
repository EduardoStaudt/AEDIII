...
// EX7
// verifica se a arvore toda � AVL (fator <= 1 em todos os nos)
int ehAVL_ArvAVL(ArvAVL *raiz){
    if(raiz == NULL)
        return 1;
    if(*raiz == NULL)
        return 1;
    // se o fator de balanceamento for maior que 1 nao � AVL
    if(fatorBalanceamento_NO(*raiz) > 1)
        return 0;
    int esq = ehAVL_ArvAVL(&((*raiz)->esq));
    int dir = ehAVL_ArvAVL(&((*raiz)->dir));
    return (esq && dir);
}

// EX8
// percorre a arvore antiga em ordem e insere tudo numa AVL nova
void copia_emOrdem(ArvAVL *raiz, ArvAVL *nova){
    if(raiz == NULL)
        return;
    if(*raiz == NULL)
        return;
    copia_emOrdem(&((*raiz)->esq), nova);
    insere_ArvAVL(nova, (*raiz)->info);
    copia_emOrdem(&((*raiz)->dir), nova);
}

ArvAVL* transforma(ArvAVL *raiz){
    ArvAVL* nova = cria_ArvAVL();
    copia_emOrdem(raiz, nova);
    return nova;
}